...
<template>
    <div class="layout">
		...
		<!--查看字典值-->
		<Modal v-model="dicItem.show" width="900" :mask-closable=false>
			<p slot="header" style="text-align:center">
				<span>内部值</span>
			</p>
			<div v-if="dicItem.show">
				<dic-item :dicId="dicItem.dicId"></dic-item>
			</div>
			<Row slot="footer">
				<Button type="ghost" size="large" @click="dicItem.show = false">关闭</Button>
			</Row>
		</Modal>
...
<script>
    ...
    import DicItem from './dicItem.vue';
    export default {
        name:'sysagentRole',
		components:{
        	DicItem
        ...
        methods:{
            ...
            edit(params){
				this.registerAgent={};
                const self = this;
                Util.ajax.post(this.apiUrlPrefix+"info?id="+params.row.id).then(function (response) {
                    if(response.data.code=="100"){
                        self.clearFormItem();
                        //self.formItem = response.data.agent;
                        self.$set(self.$data,'formItem',response.data.formItem);
			...
			//显示值
			showDicItem(param){
            	this.dicItem.show=true;
            	this.dicItem.dicId = param.row.id;
			},
			//搜索重置
			searchInit(){
				this.page.nowPage=1;
