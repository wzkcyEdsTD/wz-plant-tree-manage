<template>
	<div class="layout">
			...
			<div>
				<Form :model="formItem" :label-width="80">
					...
					<FormItem label="语音上传">
						<Row>
							<Col :span=14>
							<Button type="info" icon="upload" @click.native="audioUpModal.show=true">选择音频</Button>
							<span style="font-size: 18px;">{{formItem.audio}}</span>
							</Col>
						</Row>
					</FormItem>
					<FormItem label="二维码" >
						<qrcode :size=120 :value="'http://yuanlin.ruixing.cc/'+formItem.audio"></qrcode>
					</FormItem>
				</Form>
			</div>
			<Row slot="footer">
		...
		<!--音频上传-->
		<Modal v-model="audioUpModal.show" width="900">
			<p slot="header" style="text-align:center">
				<span>音频选择上传</span>
			</p>
			<audio-upload @chooseAudio="chooseAudio"></audio-upload>
			<Row slot="footer">
				<Button type="ghost" size="large" @click="audioUpModal.show = false">关闭</Button>
				<Button type="info" size="large" @click="confirmAudio">确定</Button>
			</Row>
		</Modal>
	</div>
</template>
<script>
	...
	import AudioUpload from '@/views/yuanlin/park/audio.vue';
	export default {
		components:{
			Qrcode,AudioUpload
		},
		data () {
			return {
				//接口前缀
				apiUrlPrefix:"/api/sys/yl/parkaudio/",
				uploadShow:false,
				modal:{
					show:false,
					isadd:true
				...
				columns1: [
					...
					},
					{
						...
						render:(h,params)=>{
							return h(Qrcode,{
								props:{
									value:'http://yuanlin.ruixing.cc/'+params.row.audio,
									size:80
								}
							});
						}
		...
		methods:{
			...
			//音频选择
			chooseAudio(audio){
				this.audioUpModal.chooseAudio = audio;
			},
			confirmAudio(){
				if(this.audioUpModal.chooseAudio){
					this.formItem.ad = this.audioUpModal.chooseAudio;
					this.formItem.audio = this.audioUpModal.chooseAudio.path;
					this.audioUpModal.chooseAudio = null;
				}
				this.audioUpModal.show = false;
			}
		},
		mounted:function () {
			this.init();
