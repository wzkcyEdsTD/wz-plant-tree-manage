<style>
	.mce-floatpanel{z-index: 1200!important;}
</style>
<template>
	<div>
		<Card shadow>
		...
		<image-upload :modalShow="isMyChoose"
					  @closemodal="closeImageModal"
					  @chooseUpdate="chooseUpdate"
					  :isChooseOne=false></image-upload>
	</div>
</template>
<script>
		...
		methods: {
			init () {
				this.$nextTick(() => {
					...
					tinymce.init({
						...
						toolbar1: ' newnote print preview | undo redo | insert | styleselect | forecolor backcolor bold italic fontselect fontsizeselect | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image emoticons media codesample fullscreen',
			...
			chooseUpdate(arr){
				...
				arr.forEach(function (item) {
					html+=`<img src="/${item.path}" style="max-width:100%;height:auto;"><br/>`
				});
